...
public class LocationController extends Controller implements IConst {
    
    public static Result locationPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Location>> msg = BaseController.doGetAll("Location",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(location.render(msg.data));
        } else {
    ...
    public static Result locationBackendPage() {
        return ok(location_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_Location")
    @Role("create_location")
    public static Result add() {
        Msg<Location> msg = new Msg<>();

        Form<LocationParser> httpForm = form(LocationParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            LocationParser formObj = httpForm.get();            
            Location newObj = new Location();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Location", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Location", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("location_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_Location")
    @Role("update_location")
    public static Result update(long id) {
        Msg<Location> msg = new Msg<>();

        Location found = Location.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Location", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Location", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class LocationParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Location.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Location>> msg = BaseController.doGetAll("Location",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Location.find.findRowCount());
		List<Location> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Location.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Location.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Location.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Location.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Location.class, "createdAt"));//created_at
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Location.class, "comment"));//comment
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_Location")
    @Role("report_location")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Location.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
