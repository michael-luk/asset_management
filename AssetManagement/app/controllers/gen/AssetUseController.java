...
public class AssetUseController extends Controller implements IConst {
    
    public static Result assetUsePage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<AssetUse>> msg = BaseController.doGetAll("AssetUse",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(asset_use.render(msg.data));
        } else {
    ...
    public static Result assetUseBackendPage() {
        return ok(asset_use_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_AssetUse")
    @Role("create_asset_use")
    public static Result add() {
        Msg<AssetUse> msg = new Msg<>();

        Form<AssetUseParser> httpForm = form(AssetUseParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            AssetUseParser formObj = httpForm.get();            
            AssetUse newObj = new AssetUse();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetUse", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetUse", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("asset_use_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_AssetUse")
    @Role("update_asset_use")
    public static Result update(long id) {
        Msg<AssetUse> msg = new Msg<>();

        AssetUse found = AssetUse.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetUse", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetUse", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class AssetUseParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(AssetUse.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<AssetUse>> msg = BaseController.doGetAll("AssetUse",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, AssetUse.find.findRowCount());
		List<AssetUse> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(AssetUse.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(AssetUse.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(AssetUse.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(AssetUse.class, "content"));//content
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(AssetUse.class, "status"));//status
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(AssetUse.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(AssetUse.class, "createdAt"));//created_at
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(AssetUse.class, "comment"));//comment
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(AssetUse.class, "user"));//user_id
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell2.setCellValue(EnumInfoReader.getEnumName(AssetUse.class, "status", item.status));
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_AssetUse")
    @Role("report_asset_use")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(AssetUse.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
