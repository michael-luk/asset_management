...
public class AssetTypeController extends Controller implements IConst {
    
    public static Result assetTypePage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<AssetType>> msg = BaseController.doGetAll("AssetType",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(asset_type.render(msg.data));
        } else {
    ...
    public static Result assetTypeBackendPage() {
        return ok(asset_type_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_AssetType")
    @Role("create_asset_type")
    public static Result add() {
        Msg<AssetType> msg = new Msg<>();

        Form<AssetTypeParser> httpForm = form(AssetTypeParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            AssetTypeParser formObj = httpForm.get();            
            AssetType newObj = new AssetType();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetType", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetType", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("asset_type_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_AssetType")
    @Role("update_asset_type")
    public static Result update(long id) {
        Msg<AssetType> msg = new Msg<>();

        AssetType found = AssetType.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetType", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetType", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class AssetTypeParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(AssetType.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<AssetType>> msg = BaseController.doGetAll("AssetType",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, AssetType.find.findRowCount());
		List<AssetType> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(AssetType.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(AssetType.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(AssetType.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(AssetType.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(AssetType.class, "createdAt"));//created_at
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(AssetType.class, "comment"));//comment
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_AssetType")
    @Role("report_asset_type")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(AssetType.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
