...
public class DictController extends Controller implements IConst {
    
    public static Result dictPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Dict>> msg = BaseController.doGetAll("Dict",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(dict.render(msg.data));
        } else {
    ...
    public static Result dictBackendPage() {
        return ok(dict_backend.render());
    }
    ...
    public static Result add() {
        Msg<Dict> msg = new Msg<>();

        Form<DictParser> httpForm = form(DictParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            DictParser formObj = httpForm.get();            
            Dict newObj = new Dict();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Dict", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Dict", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("dict_backend", "new");
    ...
    public static Result update(long id) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Dict", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Dict", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class DictParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Dict.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Dict>> msg = BaseController.doGetAll("Dict",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Dict.find.findRowCount());
		List<Dict> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Dict.class) + "报表");
		// 设置列
        sheet2.setColumnWidth(0, 4000);
        sheet2.setDefaultColumnStyle(0, cellStyle);//name
        sheet2.setColumnWidth(1, 4000);
        sheet2.setDefaultColumnStyle(1, cellStyle);//code
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Dict.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Dict.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Dict.class, "code"));//code
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Dict.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Dict.class, "createdAt"));//created_at
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Dict.class, "comment"));//comment
	...
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Dict.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
