...
public class MemberController extends Controller implements IConst {
    ...
    public static Result memberPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Member>> msg = BaseController.doGetAll("Member",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(member.render(msg.data));
        } else {
    ...
    public static Result memberBackendPage() {
        return ok(member_backend.render());
    }
    
    public static Result getMemberUsers(Long refId, Integer page, Integer size) {
        if (size == 0)
            size = PAGE_SIZE;
        if (page <= 0)
            page = 1;
        ...
        Member found = Member.find.byId(refId);
        if (found != null) {
            if (found.users.size() > 0) {
                Page<User> records;
                records = User.find.where().eq("members.id", refId).orderBy("id desc").findPagingList(size)
            ...
            msg.message = NO_FOUND;
            play.Logger.info("member result: " + NO_FOUND);
        }
        return ok(Json.toJson(msg));
    }

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_Member")
    @Role("create_member")
    public static Result add() {
        Msg<Member> msg = new Msg<>();

        Form<MemberParser> httpForm = form(MemberParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            MemberParser formObj = httpForm.get();            
            Member newObj = new Member();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Member", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Member", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            }
            ...
            try{
                ...
                sendWebSocketByChannelTag("member_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_Member")
    @Role("update_member")
    public static Result update(long id) {
        Msg<Member> msg = new Msg<>();

        Member found = Member.find.byId(id);
        if (found == null) {
            msg.message = NO_FOUND;
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Member", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Member", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            try{
                ...
                // 处理多对多 member <-> User, 先清掉对面的
                for (User refObj : found.users) {
                    if (refObj.members.contains(found)) {
                        refObj.members.remove(found);
                        refObj.save();
                    }
                }
                    ...
                    for (User jsonRefObj : formObj.users) {
                        for (User dbRefObj : allRefUsers) {
                            if (dbRefObj.id == jsonRefObj.id) {
                                ...
                                if (!dbRefObj.members.contains(found)) {
                                    dbRefObj.members.add(found);
                                    dbRefObj.save();
                                }
                            }
    ...
    public static class MemberParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Member.class, "name") + "不能为空";
            }
    ...
    public static Result delete(long id) {
        ...
        if (found != null) {
            ...
            try{
                ...
                sendWebSocketByChannelTag("member_backend", "delete");
            } catch (PersistenceException ex){
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Member>> msg = BaseController.doGetAll("Member",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Member.find.findRowCount());
		List<Member> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Member.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Member.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Member.class, "isDefault"));//is_default
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Member.class, "name"));//name
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Member.class, "code"));//code
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Member.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Member.class, "createdAt"));//created_at
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(Member.class, "comment"));//comment
	...
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Member.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
