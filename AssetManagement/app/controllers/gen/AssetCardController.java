...
public class AssetCardController extends Controller implements IConst {
    
    public static Result assetCardPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<AssetCard>> msg = BaseController.doGetAll("AssetCard",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(asset_card.render(msg.data));
        } else {
    ...
    public static Result assetCardBackendPage() {
        return ok(asset_card_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_AssetCard")
    @Role("create_asset_card")
    public static Result add() {
        Msg<AssetCard> msg = new Msg<>();

        Form<AssetCardParser> httpForm = form(AssetCardParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            AssetCardParser formObj = httpForm.get();            
            AssetCard newObj = new AssetCard();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetCard", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetCard", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("asset_card_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_AssetCard")
    @Role("update_asset_card")
    public static Result update(long id) {
        Msg<AssetCard> msg = new Msg<>();

        AssetCard found = AssetCard.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetCard", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetCard", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class AssetCardParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(AssetCard.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<AssetCard>> msg = BaseController.doGetAll("AssetCard",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, AssetCard.find.findRowCount());
		List<AssetCard> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(AssetCard.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(AssetCard.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(AssetCard.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(AssetCard.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(AssetCard.class, "createdAt"));//created_at
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(AssetCard.class, "comment"));//comment
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_AssetCard")
    @Role("report_asset_card")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(AssetCard.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
