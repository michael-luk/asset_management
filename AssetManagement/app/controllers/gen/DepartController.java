...
public class DepartController extends Controller implements IConst {
    
    public static Result departPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Depart>> msg = BaseController.doGetAll("Depart",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(depart.render(msg.data));
        } else {
    ...
    public static Result departBackendPage() {
        return ok(depart_backend.render());
    }
    
    public static Result getDepartMaterials(Long refId, Integer page, Integer size) {
        if (size == 0)
            size = PAGE_SIZE;
        if (page <= 0)
            page = 1;

        Msg<List<Material>> msg = new Msg<>();

        Depart found = Depart.find.byId(refId);
        if (found != null) {
            if (found.materials.size() > 0) {
                Page<Material> records;
                records = Material.find.where().eq("departs.id", refId).orderBy("id desc").findPagingList(size)
                ...
                if (records.getTotalRowCount() > 0) {
                    ...
                    play.Logger.info("materials row result: " + NO_FOUND);
                }
            } else {
                msg.message = NO_FOUND;
                play.Logger.info("materials result: " + NO_FOUND);
            }
        } else {
            msg.message = NO_FOUND;
            play.Logger.info("depart result: " + NO_FOUND);
        }
        return ok(Json.toJson(msg));
    }
    public static Result getDepartUsers(Long refId, Integer page, Integer size) {
        if (size == 0)
            size = PAGE_SIZE;
        if (page <= 0)
            page = 1;
        ...
        if (found != null) {
            if (found.users.size() > 0) {
                Page<User> records;
                records = User.find.where().eq("departs.id", refId).orderBy("id desc").findPagingList(size)
                        .setFetchAhead(false).getPage(page - 1);
    ...
    @MethodName("新增_Depart")
    @Role("create_depart")
    public static Result add() {
        Msg<Depart> msg = new Msg<>();

        Form<DepartParser> httpForm = form(DepartParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            DepartParser formObj = httpForm.get();            
            Depart newObj = new Depart();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Depart", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Depart", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            }
            ...
            if (formObj.materials == null) {
                formObj.materials = new ArrayList<>();
            }
            newObj.materials = formObj.materials;
            ...
            try{
                ...
                for (Material jsonRefObj : formObj.materials){
                    Material dbRefObj = Material.find.byId(jsonRefObj.id);
                    dbRefObj.departs.add(newObj);
                    dbRefObj.save();
                }
                ...
                sendWebSocketByChannelTag("depart_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_Depart")
    @Role("update_depart")
    public static Result update(long id) {
        Msg<Depart> msg = new Msg<>();

        Depart found = Depart.find.byId(id);
        if (found == null) {
            msg.message = NO_FOUND;
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Depart", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Depart", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            try{
                ...
                // 处理多对多 depart <-> Material, 先清掉对面的
                for (Material refObj : found.materials) {
                    if (refObj.departs.contains(found)) {
                        refObj.departs.remove(found);
                        refObj.save();
                    }
                }

                // 清掉自己这边的
                found.materials = new ArrayList<>();
                found.save();

                // 两边加回
                List<Material> allRefMaterials = Material.find.all();
                if (formObj.materials != null) {
                    for (Material jsonRefObj : formObj.materials) {
                        for (Material dbRefObj : allRefMaterials) {
                            if (dbRefObj.id == jsonRefObj.id) {
                                if (!found.materials.contains(dbRefObj)) {
                                    found.materials.add(dbRefObj);
                                }
                                if (!dbRefObj.departs.contains(found)) {
                                    dbRefObj.departs.add(found);
                                    dbRefObj.save();
                                }
                            }
    ...
    public static class DepartParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Depart.class, "name") + "不能为空";
            }
    ...
    public static Result delete(long id) {
        ...
        if (found != null) {
            ...
            try{
                // 解除多对多的关联
                for (Material material : found.materials) {
                    material.departs.remove(found);
                    material.save();
                }
                found.materials = new ArrayList<>();
                for (User user : found.users) {
                    user.departs.remove(found);
                    user.save();
                }
                found.users = new ArrayList<>();
                ...
                sendWebSocketByChannelTag("depart_backend", "delete");
            } catch (PersistenceException ex){
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Depart>> msg = BaseController.doGetAll("Depart",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Depart.find.findRowCount());
		List<Depart> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Depart.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Depart.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Depart.class, "isDefault"));//is_default
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Depart.class, "name"));//name
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Depart.class, "code"));//code
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Depart.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Depart.class, "createdAt"));//created_at
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(Depart.class, "comment"));//comment
	...
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Depart.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
