...
public class FixRequestController extends Controller implements IConst {
    
    public static Result fixRequestPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<FixRequest>> msg = BaseController.doGetAll("FixRequest",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(fix_request.render(msg.data));
        } else {
    ...
    public static Result fixRequestBackendPage() {
        return ok(fix_request_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_FixRequest")
    @Role("create_fix_request")
    public static Result add() {
        Msg<FixRequest> msg = new Msg<>();

        Form<FixRequestParser> httpForm = form(FixRequestParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            FixRequestParser formObj = httpForm.get();            
            FixRequest newObj = new FixRequest();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("FixRequest", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("FixRequest", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("fix_request_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_FixRequest")
    @Role("update_fix_request")
    public static Result update(long id) {
        Msg<FixRequest> msg = new Msg<>();

        FixRequest found = FixRequest.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("FixRequest", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("FixRequest", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class FixRequestParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(FixRequest.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<FixRequest>> msg = BaseController.doGetAll("FixRequest",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, FixRequest.find.findRowCount());
		List<FixRequest> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(FixRequest.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(FixRequest.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(FixRequest.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(FixRequest.class, "content"));//content
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(FixRequest.class, "status"));//status
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(FixRequest.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(FixRequest.class, "createdAt"));//created_at
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(FixRequest.class, "comment"));//comment
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(FixRequest.class, "user"));//user_id
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell2.setCellValue(EnumInfoReader.getEnumName(FixRequest.class, "status", item.status));
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_FixRequest")
    @Role("report_fix_request")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(FixRequest.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
