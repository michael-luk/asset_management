...
public class AssetHandleController extends Controller implements IConst {
    
    public static Result assetHandlePage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<AssetHandle>> msg = BaseController.doGetAll("AssetHandle",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(asset_handle.render(msg.data));
        } else {
    ...
    public static Result assetHandleBackendPage() {
        return ok(asset_handle_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_AssetHandle")
    @Role("create_asset_handle")
    public static Result add() {
        Msg<AssetHandle> msg = new Msg<>();

        Form<AssetHandleParser> httpForm = form(AssetHandleParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            AssetHandleParser formObj = httpForm.get();            
            AssetHandle newObj = new AssetHandle();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetHandle", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetHandle", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("asset_handle_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_AssetHandle")
    @Role("update_asset_handle")
    public static Result update(long id) {
        Msg<AssetHandle> msg = new Msg<>();

        AssetHandle found = AssetHandle.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("AssetHandle", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("AssetHandle", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            Transaction txn = Ebean.beginTransaction();
            try{
                found = AssetHandle.find.byId(id);
    ...
    public static class AssetHandleParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(AssetHandle.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<AssetHandle>> msg = BaseController.doGetAll("AssetHandle",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, AssetHandle.find.findRowCount());
		List<AssetHandle> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(AssetHandle.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(AssetHandle.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(AssetHandle.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(AssetHandle.class, "status"));//status
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(AssetHandle.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(AssetHandle.class, "createdAt"));//created_at
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(AssetHandle.class, "comment"));//comment
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell1.setCellValue(EnumInfoReader.getEnumName(AssetHandle.class, "status", item.status));
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_AssetHandle")
    @Role("report_asset_handle")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(AssetHandle.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";
