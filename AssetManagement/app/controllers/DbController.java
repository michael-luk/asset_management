...
public class DbController extends Controller implements IConst {
    ...
    public static String bakFileReceiveEmail;
    ...
    @Security.Authenticated(SecuredAdmin.class)
    public static Result dbBackupAndSendEmail(String dbType) {
        String result = doDbBackupAndSendEmail(dbType);
        if ("ok".equals(result)) {
            return ok("db backup email sent to: " + bakFileReceiveEmail);
        } else {
            return ok(result);
        }
    }
...
//        if (!Application.taskFlag) return "";
    ...
    public static String doDbBackupAndSendEmail(String dbType) {
        String bakFileFullPath = doDbBackup(dbType);
        if (StrUtil.isNotNull(bakFileFullPath)) {
            ...
            if (StrUtil.isNotNull(bakFileReceiveEmail)) {
                EmailController.sendMail(bakFileReceiveEmail,
                ...
                return "dbbak.receive.email is null";
            }
        } else {
            return "db backup file path is null";
        }
    }
}
