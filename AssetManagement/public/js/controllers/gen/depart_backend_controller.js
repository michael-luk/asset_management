var app = angular.module('DepartBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);

var checkboxTemplateDepart = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
//var friendButtonTemplateDepart = '<div align="center" style="height:26px;line-height:24px"><a href="#" data-toggle="tooltip" title="跳转"><button class="btn btn-xs btn-primary" ng-click="grid.appScope.goToFriendPage(row.entity.id)"><i class="icon-share icon-white"></i></button></a></div>';
var friendButtonTemplateDepartMaterial = '<div align="center" style="height:26px;line-height:24px"><a href="#" data-toggle="tooltip" title="弹窗显示"><button class="btn btn-xs btn-success" ng-click="grid.appScope.popFriendMaterial(row.entity)"><i class="icon-list-alt icon-white"></i></button></a> <a href="#" data-toggle="tooltip" title="跳转"><button class="btn btn-xs btn-primary" ng-click="grid.appScope.goToFriendPageMaterial(row.entity.id)"><i class="icon-share icon-white"></i></button></a></div>';
//var friendButtonTemplateDepart = '<div align="center" style="height:26px;line-height:24px"><a href="#" data-toggle="tooltip" title="跳转"><button class="btn btn-xs btn-primary" ng-click="grid.appScope.goToFriendPage(row.entity.id)"><i class="icon-share icon-white"></i></button></a></div>';
var friendButtonTemplateDepartUser = '<div align="center" style="height:26px;line-height:24px"><a href="#" data-toggle="tooltip" title="弹窗显示"><button class="btn btn-xs btn-success" ng-click="grid.appScope.popFriendUser(row.entity)"><i class="icon-list-alt icon-white"></i></button></a> <a href="#" data-toggle="tooltip" title="跳转"><button class="btn btn-xs btn-primary" ng-click="grid.appScope.goToFriendPageUser(row.entity.id)"><i class="icon-share icon-white"></i></button></a></div>';
var readonlyImageTemplateDepart = '<div><div ng-repeat="imageName in MODEL_COL_FIELD.split(\',\')"><div ng-show="imageName"><a class="fancybox" target="_blank" data-fancybox-group="gallery" fancybox ng-href="/showImage/{{imageName}}"><img ng-src="/showimg/thumb/{{imageName}}" style="width:50px;height:50px;float:left"></a></div></div></div>';
var readonlyCheckboxTemplateDepart = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" disabled="disabled" /></div>';
...
app.controller('DepartBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isDepart = true;
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'isDefault', displayName: objFieldInfo.Depart.isDefault, headerTooltip: '点击表头可进行排序, 通过直接勾选操作来更新数据', enableCellEdit: false, cellTemplate: checkboxTemplateDepart},
        {field: 'name', displayName: objFieldInfo.Depart.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'code', displayName: objFieldInfo.Depart.code, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastUpdateTime', displayName: objFieldInfo.Depart.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.Depart.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.Depart.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'friendMaterial', displayName: objFieldInfo.Depart.materials, width: '80', headerTooltip: '弹窗/跳转显示', enableCellEdit: false, cellTemplate: friendButtonTemplateDepartMaterial},
        {field: 'friendUser', displayName: objFieldInfo.Depart.users, width: '80', headerTooltip: '弹窗/跳转显示', enableCellEdit: false, cellTemplate: friendButtonTemplateDepartUser},

    ];
    
    $scope.goToFriendPageMaterial = function(pid) { location = '/admin/material?relate=departs.id&relateValue=' + pid }
    $scope.goToFriendPageUser = function(pid) { location = '/admin/user?relate=departs.id&relateValue=' + pid }
    
    $scope.friends4gridMaterial = []
    $scope.pageInfo4FriendMaterial = {}
    $scope.searchFieldNameMaterial = searchFieldNameMaterial
    $scope.searchFieldNameMaterialComment = searchFieldNameMaterialComment

    $scope.$watch('paginationConf4FriendMaterial.itemsPerPage', function(newValue, oldValue, scope){
        if (newValue != oldValue) {
            fillGridWithFriendsMaterial();
        }
    }, false);

    $scope.$watch('paginationConf4FriendMaterial.currentPage', function(newValue, oldValue, scope){
        if (newValue != oldValue) {
            fillGridWithFriendsMaterial();
        }
    }, false);

    $scope.paginationConf4FriendMaterial = {
        currentPage: 1, //首页
        itemsPerPage: 10, //每页显示数量
        pagesLength: 5,  //显示多少个页数格子
        perPageOptions: [1, 2, 5, 10, 20, 50, 100],//选择每页显示数量
        rememberPerPage: 'itemsPerPage4FriendMaterial'
    };
    
    function fillGridWithFriendsMaterial() {
        url = '/base/Material/all?page=' 
                    + $scope.paginationConf4FriendMaterial.currentPage 
                    + '&size=' + $scope.paginationConf4FriendMaterial.itemsPerPage
                    
        if ($scope.currentObj.queryKeyword4Material)
            url += '&searchOn=' + $scope.searchFieldNameMaterial + '&kw=' + $scope.currentObj.queryKeyword4Material
            
        $http.get(url)
            .success(function (data, status, headers, config) {
            if (data.flag) {
                $scope.pageInfo4FriendMaterial = data.page;
                $scope.paginationConf4FriendMaterial.totalItems = data.page.total;
                
                if ($scope.currentObj.id) {
                    var allFriends = data.data;
                    
                    //用于比较, 全取不分页
                    $http.get('/depart/' + $scope.currentObj.id + '/materials?page=1&size=1000000')
                    .success(function (data, status, headers, config) {
                        if (data.flag){
                            for (x in allFriends) {
                        ...
                        $scope.friends4gridMaterial = allFriends;
                    })
                }
                else {
                    $scope.friends4gridMaterial = data.data;
                }
            }
            else {
                $scope.parentMaterials4grid = [];
                //showAlert('错误: ', data.message, 'danger')
            }
        });
    }
    
    $scope.myFriendSelectionsMaterial = [];
    $scope.gridFriendsMaterial = {
        data: 'friends4gridMaterial',
        enableRowSelection: true,
        enableRowHeaderSelection: false,
        multiSelect: true,
        onRegisterApi: function (gridApi) {
            $scope.gridApi = gridApi;
            gridApi.selection.on.rowSelectionChanged($scope, function (rows) {
                $scope.myFriendSelectionsMaterial = gridApi.selection.getSelectedRows();
            });
        },
    ...
    $scope.searchContent4Material = function(){
        if($scope.paginationConf4FriendMaterial.currentPage != 1){
            $scope.paginationConf4FriendMaterial.currentPage = 1
        }
        else{
            fillGridWithFriendsMaterial()
        }
        ...
        $http.get(url)
            ...
            if (data.flag) {
                ...
                if ($scope.currentObj.id) {
                    ...
                    //用于比较, 全取不分页
                    $http.get('/depart/' + $scope.currentObj.id + '/users?page=1&size=1000000')
                    .success(function (data, status, headers, config) {
                        if (data.flag){
                            for (x in allFriends) {
                                allFriends[x].refFriend = false
    ...
    $scope.saveContent = function() {
        ...
        if ($scope.myFriendSelectionsMaterial.length > 0) content.materials = $scope.myFriendSelectionsMaterial
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/depart/' + content.id
        ...
        $http.get('/new/depart')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    ...
                    createDialogService("/assets/html/edit_templates/depart.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/depart?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// friend Material popup show //////////
    
    $scope.gridFriendMaterial = {
        data: 'friendMaterial4grid',
    ...
    $scope.gridFriendMaterial.columnDefs = [        
        {field: 'id', displayName: 'Id', width: '40', enableCellEdit: false},
        {field: 'name', displayName: objFieldInfo.Material.name, enableCellEdit: true},
        {field: 'content', displayName: objFieldInfo.Material.content, enableCellEdit: true},
        {field: 'quantity', displayName: objFieldInfo.Material.quantity, enableCellEdit: true},
        {field: 'price', displayName: objFieldInfo.Material.price, enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.Material.status, enableCellEdit: false, cellTemplate: '<span ng-bind="grid.appScope.objEnumInfo.Material.status[MODEL_COL_FIELD]"></span>'},
        {field: 'lastUpdateTime', displayName: objFieldInfo.Material.lastUpdateTime, enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.Material.createdAt, enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.Material.comment, enableCellEdit: true},
    ];

    $scope.popFriendMaterial = function (obj) {
        if (obj) {
            $scope.currentObj = obj;

            fillGridWithFriendMaterial()

            createDialogService("/assets/html/child_pop_templates/depart_2_material.html", {
                id: 'friend_material',
                title: '查看',
                scope: $scope,
                footerTemplate: '<div></div>'
    ...
    function fillGridWithFriendMaterial() {
        $scope.friendMaterial4grid = []
        $http.get('/base/Material/all?page='
        + $scope.paginationConf4FriendMaterial.currentPage
        + '&size=' + $scope.paginationConf4FriendMaterial.itemsPerPage
        + '&fieldOn=departs.id&fieldValue=' + $scope.currentObj.id)
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    $scope.friendMaterial4grid = data.data;
                    $scope.pageInfo4friendMaterial = data.page;
                    $scope.paginationConf4FriendMaterial.totalItems = data.page.total;
                }
            });
    ...
    $scope.gridFriendUser.columnDefs = [        
        ...
        {field: 'isEmailVerified', displayName: objFieldInfo.User.isEmailVerified, width: 120, enableCellEdit: false, cellTemplate: readonlyCheckboxTemplateDepart},
        ...
        {field: 'images', displayName: objFieldInfo.User.images, width: 170, enableCellEdit: false, cellTemplate: readonlyImageTemplateDepart},
    ...
    $scope.popFriendUser = function (obj) {
        if (obj) {
            ...
            createDialogService("/assets/html/child_pop_templates/depart_2_user.html", {
                id: 'friend_user',
                title: '查看',
                scope: $scope,
                footerTemplate: '<div></div>'
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'depart_backend_' + getNowFormatDate()
