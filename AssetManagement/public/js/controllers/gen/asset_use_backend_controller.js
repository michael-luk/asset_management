var app = angular.module('AssetUseBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);

var checkboxTemplateAssetUse = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
...
app.controller('AssetUseBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isAssetUse = true;
    ...
    dropdownTemplateAssetUseStatus = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.AssetUse.status).length; i++) {
        $scope.status.push({"id": i, "name": objEnumInfo.AssetUse.status[i]})
        dropdownTemplateAssetUseStatus += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.AssetUse.status[i] + '</option>'
    }
    dropdownTemplateAssetUseStatus += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.AssetUse.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'content', displayName: objFieldInfo.AssetUse.content, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.AssetUse.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateAssetUseStatus},
        {field: 'lastUpdateTime', displayName: objFieldInfo.AssetUse.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.AssetUse.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.AssetUse.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'refUserId', displayName: objFieldInfo.AssetUse.refUserId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},

        {field: 'user', displayName: objFieldInfo.AssetUse.user, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
    ];
    ...
    function refreshData(showMsg){
        var url = '/base/AssetUse/all?page=' 
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/AssetUse/' + content.id
        ...
        $http.get('/new/assetuse')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    ...
                    createDialogService("/assets/html/edit_templates/asset_use.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/assetuse?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'asset_use_backend_' + getNowFormatDate()
