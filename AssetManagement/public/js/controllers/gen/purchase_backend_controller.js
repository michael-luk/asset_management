...
app.controller('PurchaseBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'content', displayName: objFieldInfo.Purchase.content, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        ...
        {field: 'refMaterialId', displayName: objFieldInfo.Purchase.refMaterialId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/material?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},
        ...
        {field: 'material', displayName: objFieldInfo.Purchase.material, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/material?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/Purchase/' + content.id
