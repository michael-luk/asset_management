...
app.controller('AssetCardBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isAssetCard = true;
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.AssetCard.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'images', displayName: objFieldInfo.AssetCard.images, width: 170, enableCellEdit: false, cellTemplate: uploadImageTemplateAssetCard},
        {field: 'lastUpdateTime', displayName: objFieldInfo.AssetCard.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.AssetCard.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.AssetCard.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
    ...
    function refreshData(showMsg){
        var url = '/base/AssetCard/all?page=' 
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/AssetCard/' + content.id
    ...
    $scope.addContent = function(){
        $http.get('/new/assetcard')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    $scope.currentObj = data.data;
                    
                    
                    createDialogService("/assets/html/edit_templates/asset_card.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/assetcard?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'asset_card_backend_' + getNowFormatDate()
