var app = angular.module('MemberBackendApp', ['tm.pagination', 'ui.grid', 'ui.grid.resizeColumns', 'ui.grid.selection', 'ui.grid.edit', 'angularFileUpload', 'fundoo.services', 'simditor', 'ui.grid.autoFitColumns']);

var checkboxTemplateMember = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" ng-click="grid.appScope.updateEntity(col, row)" /></div>';
//var friendButtonTemplateMember = '<div align="center" style="height:26px;line-height:24px"><a href="#" data-toggle="tooltip" title="跳转"><button class="btn btn-xs btn-primary" ng-click="grid.appScope.goToFriendPage(row.entity.id)"><i class="icon-share icon-white"></i></button></a></div>';
var friendButtonTemplateMemberUser = '<div align="center" style="height:26px;line-height:24px"><a href="#" data-toggle="tooltip" title="弹窗显示"><button class="btn btn-xs btn-success" ng-click="grid.appScope.popFriendUser(row.entity)"><i class="icon-list-alt icon-white"></i></button></a> <a href="#" data-toggle="tooltip" title="跳转"><button class="btn btn-xs btn-primary" ng-click="grid.appScope.goToFriendPageUser(row.entity.id)"><i class="icon-share icon-white"></i></button></a></div>';
var readonlyImageTemplateMember = '<div><div ng-repeat="imageName in MODEL_COL_FIELD.split(\',\')"><div ng-show="imageName"><a class="fancybox" target="_blank" data-fancybox-group="gallery" fancybox ng-href="/showImage/{{imageName}}"><img ng-src="/showimg/thumb/{{imageName}}" style="width:50px;height:50px;float:left"></a></div></div></div>';
var readonlyCheckboxTemplateMember = '<div><input type="checkbox" ng-model="MODEL_COL_FIELD" disabled="disabled" /></div>';
...
app.controller('MemberBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isMember = true;
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'isDefault', displayName: objFieldInfo.Member.isDefault, headerTooltip: '点击表头可进行排序, 通过直接勾选操作来更新数据', enableCellEdit: false, cellTemplate: checkboxTemplateMember},
        {field: 'name', displayName: objFieldInfo.Member.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'code', displayName: objFieldInfo.Member.code, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'lastUpdateTime', displayName: objFieldInfo.Member.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.Member.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.Member.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'friendUser', displayName: objFieldInfo.Member.users, width: '80', headerTooltip: '弹窗/跳转显示', enableCellEdit: false, cellTemplate: friendButtonTemplateMemberUser},

    ];
    
    $scope.goToFriendPageUser = function(pid) { location = '/admin/user?relate=members.id&relateValue=' + pid }
        ...
        $http.get(url)
            ...
            if (data.flag) {
                ...
                if ($scope.currentObj.id) {
                    ...
                    //用于比较, 全取不分页
                    $http.get('/member/' + $scope.currentObj.id + '/users?page=1&size=1000000')
                    .success(function (data, status, headers, config) {
                        if (data.flag){
                            for (x in allFriends) {
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/member/' + content.id
        ...
        $http.get('/new/member')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    ...
                    createDialogService("/assets/html/edit_templates/member.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/member?' + getQueryUrl()
            }
        });
    }
    ...
    $scope.gridFriendUser.columnDefs = [        
        ...
        {field: 'isEmailVerified', displayName: objFieldInfo.User.isEmailVerified, width: 120, enableCellEdit: false, cellTemplate: readonlyCheckboxTemplateMember},
        ...
        {field: 'images', displayName: objFieldInfo.User.images, width: 170, enableCellEdit: false, cellTemplate: readonlyImageTemplateMember},
    ...
    $scope.popFriendUser = function (obj) {
        if (obj) {
            ...
            createDialogService("/assets/html/child_pop_templates/member_2_user.html", {
                id: 'friend_user',
                title: '查看',
                scope: $scope,
                footerTemplate: '<div></div>'
    ...
    function fillGridWithFriendUser() {
        ...
        + $scope.paginationConf4FriendUser.currentPage
        + '&size=' + $scope.paginationConf4FriendUser.itemsPerPage
        + '&fieldOn=members.id&fieldValue=' + $scope.currentObj.id)
            .success(function (data, status, headers, config) {
                if (data.flag) {
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'member_backend_' + getNowFormatDate()
